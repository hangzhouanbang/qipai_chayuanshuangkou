package com.anbang.qipai.chayuanshuangkou.cqrs.c.domain.test;

import java.util.ArrayList;
import java.util.List;

import com.anbang.qipai.chayuanshuangkou.cqrs.c.domain.ChayuanShuangkouGongxianFen;
import com.anbang.qipai.chayuanshuangkou.cqrs.c.domain.ChayuanShuangkouXianshuBeishu;
import com.anbang.qipai.chayuanshuangkou.init.XianshuCalculatorHelper;
...
public class CaseTest1 {
	...
	public static void main(String[] args) {
		...
		ChayuanShuangkouGongxianFen fen = calculateTotalGongxianfenWithShouPaiForPlayer();
	 ...
	 * 计算玩家总贡献分
	 */
	private static ChayuanShuangkouGongxianFen calculateTotalGongxianfenWithShouPaiForPlayer() {
		int[] xianshuArray = new int[9];
		ChayuanShuangkouGongxianFen bestGongxianfen = new ChayuanShuangkouGongxianFen(xianshuArray);
		...
		if (xiaowangCount + dawangCount == 4) {// 有天王炸
			...
			xianshuCount1[2] += 1;
			ChayuanShuangkouGongxianFen gongxianfen = calculatePaiXingWithWangDang(dianshuAmountArray.clone(), 1, 0, 1,
					bx, xianshuCount1);
			...
			xianshuCount2[3] += 1;
			ChayuanShuangkouGongxianFen gongxianfen1 = calculatePaiXingWithoutWangDang(dianshuAmountArray, bx,
					xianshuCount2);
		...
		if (xiaowangCount + dawangCount == 3) {// 有三王炸
			...
			xianshuCount3[2] += 1;
			ChayuanShuangkouGongxianFen gongxianfen = calculatePaiXingWithoutWangDang(dianshuAmountArray, bx,
					xianshuCount3);
			if (bestGongxianfen == null || bestGongxianfen.getValue() < gongxianfen.getValue()) {
				bestGongxianfen = gongxianfen;
			}
		...
		if (wangCount > 0) {
			// 有王牌
			ChayuanShuangkouGongxianFen gongxianfen = calculatePaiXingWithWangDang(dianshuCountArray, wangCount,
			...
			// 没有王牌
			ChayuanShuangkouGongxianFen gongxianfen = calculatePaiXingWithoutWangDang(dianshuCountArray, bx,
					xianshuArray);
			if (bestGongxianfen == null || bestGongxianfen.getValue() < gongxianfen.getValue()) {
				bestGongxianfen = gongxianfen;
			}
	...
	private static ChayuanShuangkouGongxianFen calculatePaiXingWithoutWangDang(int[] dianshuCountArray,
	...
	private static ChayuanShuangkouGongxianFen calculatePaiXingWithWangDang(int[] dianshuCountArray, int wangCount,
		...
		for (int zuheCode = 0; zuheCode < maxZuheCode; zuheCode++) {
			...
			if (wangDangPaiArray != null) {
				...
				ChayuanShuangkouGongxianFen gongxianfen = calculateBestGongxianfen(dianshuCountArray.clone(), bx,
	 ...
	 * 根据牌型和牌数计算最佳贡献分
	 */
	private static ChayuanShuangkouGongxianFen calculateBestGongxianfen(int[] dianshuCountArray, BianXingWanFa bx,
	...
	private static void calculateGongxianfen(int[] dianshuCountArray, BianXingWanFa bx, int[] xianshuArray,
			List<ZhadanDianShuZu> zhadanDianShuZuList, ChayuanShuangkouGongxianFen bestGongxianfen) {
		if (zhadanDianShuZuList.isEmpty()) {
			ChayuanShuangkouGongxianFen gongxianfen = new ChayuanShuangkouGongxianFen(xianshuArray);
