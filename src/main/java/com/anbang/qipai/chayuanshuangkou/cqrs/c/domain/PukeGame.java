...
import com.anbang.qipai.chayuanshuangkou.cqrs.c.domain.state.PlayerVotedWhenChaodi;
import com.anbang.qipai.chayuanshuangkou.cqrs.c.domain.state.PlayerVotingWhenAfterChaodi;
import com.anbang.qipai.chayuanshuangkou.cqrs.c.domain.state.PlayerVotingWhenChaodi;
...
public class PukeGame extends FixedPlayersMultipanAndVotetofinishGame {
	...
	private void createJuAndStartFirstPan(long currentTime) throws Exception {
		...
		// 生成盘结果
		ChayuanShuangkouCurrentPanResultBuilder panResultBuilder = new ChayuanShuangkouCurrentPanResultBuilder();
		...
		// 生成局结果
		ju.setJuResultBuilder(new ChayuanShuangkouJuResultBuilder());
		...
		ChayuanShuangkouDianShuZuYaPaiSolutionCalculator dianShuZuYaPaiSolutionCalculator = new ChayuanShuangkouDianShuZuYaPaiSolutionCalculator();
		...
		ChayuanShuangkouZaDanYaPaiSolutionCalculator zaDanYaPaiSolutionCalculator = new ChayuanShuangkouZaDanYaPaiSolutionCalculator();
	...
	public PukeActionResult da(String playerId, List<Integer> paiIds, String dianshuZuheIdx, long actionTime)
		...
		for (String pid : allPlayerIds()) {
			...
			ChayuanShuangkouXianshuBeishu xianshubeishu = new ChayuanShuangkouXianshuBeishu(xianshuCount);
		...
		for (int i = 0; i < renshu; i++) {
			ChayuanShuangkouGongxianFen gongxian1 = panPlayerGongxianfenList.get(i);
			for (int j = (i + 1); j < renshu; j++) {
				ChayuanShuangkouGongxianFen gongxian2 = panPlayerGongxianfenList.get(j);
				// 结算贡献分
		...
		// 计算贡献分
		for (String pid : allPlayerIds()) {
			ChayuanShuangkouGongxianFen gongxianfen = gongxianfenMap.get(pid);
				...
				if (renshu > 2) {
					...
					for (String id : playerIds) {
						if (id.equals(pid) || id.equals(duijiaPlayerId)) {
							ChayuanShuangkouXianshuBeishu xianshubeishu = maxXianshuMap.get(id);
